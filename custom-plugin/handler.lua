...
local function set_consumer(consumer, credential)
    -- kong.log('set_consumer ', inspect(consumer), inspect(credential))
...
local function do_authentication(conf)
    ...
    local credential_cache_key = kong.db.custom_plugin:cache_key(key)
    kong.log('credential_cache_key ', credential_cache_key)

    local credential, err = cache:get(credential_cache_key, nil, load_credential, key)
    kong.log('credential ', credential)
    if err then
        return error(err)
    end
    ...
    kong.log('consumer_cache_key ', consumer_cache_key)
